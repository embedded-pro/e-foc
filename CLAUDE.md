...
- **Real-time** — `Calculate()` at 20 kHz; ≤4500 cycles at 120 MHz. No virtual dispatch, no heap, no blocking, no raw `sin`/`cos` in hot path. `#pragma GCC optimize("O3","fast-math")` scoped to the hot function with `push_options`/`pop_options` (never file-wide) + `OPTIMIZE_FOR_SPEED`.
