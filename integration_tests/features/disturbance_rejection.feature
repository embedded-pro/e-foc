Feature: Disturbance Rejection
  ...
  Rather than raise the torque until the nominal 20 A limit binds, which would
  need a step that slams the rotor, it lowers the limit until a modest step
  reaches it: the steady-state demand still fits inside the limit, so the loop
  can recover, while the transient the step provokes does not, so the recovery
  runs through a saturated command and exercises the anti-windup rather than the
  linear response.
  ...
  Scenario Outline: The <algorithm> speed loop recovers through a saturated current command
    ...
    And a motor plant with:
      | max_supported_current_ampere | 0.6 |
    And the plant response is recorded at 1000 Hz for up to 1200 samples
    And a torque step of 0.015 Nm applied 300 ms after enable
    And the motor is already calibrated
    And the motor boots in speed mode
    And the speed loop runs the <algorithm> algorithm
    When the target boots
    ...
    And the motor is enabled
    And the response is captured for 750 ms after enable
    Then the speed deviation after the torque step shall stay below 15.0 rad/s
    And the speed shall recover to within 2.0 rad/s of the setpoint within 350 ms of the torque step
    And the response shall have no dropped samples
