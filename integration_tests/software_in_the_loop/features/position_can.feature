Feature: Position Controller CAN Commands
  ...
  Scenario: CAN Stop command is ignored when position motor is already in Ready state
    When the CAN Stop command is received
    Then the state machine shall be in the Ready state

  Scenario: CAN ClearFault command is ignored when position motor is not in Fault state
    When the CAN ClearFault command is received
    Then the state machine shall be in the Ready state

  Scenario: CAN Start then Stop then Start leaves position motor stably in Enabled
    When the CAN Start command is received
    And the CAN Stop command is received
    And the CAN Start command is received
    Then the state machine shall be in the Enabled state

  # REQ-INT-004: CAN Start rejected when position motor is already in Fault state
  Scenario: CAN Start command is rejected when position motor is in Fault state
    And the enable command is issued
    And a hardware fault is raised by the platform
    When the CAN Start command is received
    Then the state machine shall be in the Fault state

  # REQ-INT-005: CAN Start command after fault recovery with valid calibration re-enables position motor
  Scenario: CAN Start command after fault recovery with valid calibration reaches Enabled in position mode
    And the enable command is issued
    And a hardware fault is raised by the platform
  ...
  # REQ-INT-006: CAN EmergencyStop in Enabled returns position motor to Ready
  Scenario: CAN EmergencyStop in Enabled returns position motor to Ready
    And the CAN Start command is received
    When the CAN EmergencyStop command is received
    Then the state machine shall be in the Ready state

  # REQ-INT-007: CAN EmergencyStop in Ready keeps position motor in Ready
  Scenario: CAN EmergencyStop in Ready keeps position motor in Ready
    When the CAN EmergencyStop command is received
    Then the state machine shall be in the Ready state
