...
{
    args::ArgumentParser parser("serial_bridge -- TCP serial bridge client.");
    args::HelpFlag help(parser, "help", "display this help menu", { 'h', "help" });
    args::ValueFlag<std::string> hostArg(parser, "ip", "server host", { "host" }, std::string("host.docker.internal"));
    args::ValueFlag<int> portArg(parser, "port", "server TCP port", { "port" }, 5000);
    ...
    try
    {
        ...
        tool::TcpClientSerial serial(network.ConnectionFactory(), ipv4, static_cast<uint16_t>(port));
