...
>
> **Diagrams**: Prefer Mermaid (` ```mermaid `) for structural diagrams (flowcharts, state machines, sequences).
> For mathematical curves, waveforms, coordinate frames, and signal-flow diagrams, use TikZ figures stored in
> `documentation/tikz/` and referenced via a ` ```{=latex} ` raw block (renders in PDF; silently dropped in HTML).
> ASCII art is acceptable for simple inline sketches.
