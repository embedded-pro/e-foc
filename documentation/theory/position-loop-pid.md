...
The baseline position controller is a pure proportional (P) regulator that converts position error
into a speed reference $\omega_m^*$ for the inner speed loop. Under cascade separation — inner
speed loop bandwidth at least 5× the position bandwidth — the closed-loop position bandwidth equals
$K_p$ directly. No parameter identification is required.
...
A proportional controller $C(s) = K_p$ closes the integrator loop:
...
- Under a constant velocity reference $\dot{\theta}_m^* \neq 0$, a pure P controller has
  non-zero steady-state tracking error. Add velocity feedforward (as in P2 / Cascade P) to
