...
TEST_F(AlgorithmPersistenceTest, apply_persisted_current_algorithm_valid_raw_selects_it)
{
    ...
    EXPECT_CALL(currentMock, ActiveCurrentAlgorithm())
        .WillOnce(Return(foc::CurrentAlgorithm::pid))
        .WillRepeatedly(Return(foc::CurrentAlgorithm::deadbeat));
...
TEST_F(AlgorithmPersistenceTest, apply_persisted_already_active_algorithm_skips_select)
{
    ...
    EXPECT_CALL(currentMock, ActiveCurrentAlgorithm()).WillRepeatedly(Return(foc::CurrentAlgorithm::pid));
...
TEST_F(AlgorithmPersistenceTest, apply_persisted_algorithm_whose_design_is_infeasible_records_what_still_runs)
{
    configData.positionAlgorithm = static_cast<uint8_t>(foc::PositionAlgorithm::lqr);
    EXPECT_CALL(positionMock, ActivePositionAlgorithm()).WillRepeatedly(Return(foc::PositionAlgorithm::pid));
    EXPECT_CALL(positionMock, SelectPositionAlgorithm(foc::PositionAlgorithm::lqr))
        .WillOnce(Return(foc::SelectResult::invalidParameters));

    persistence.ApplyPersistedAlgorithms(nullptr, nullptr, &positionMock);

    EXPECT_EQ(configData.positionAlgorithm, static_cast<uint8_t>(foc::PositionAlgorithm::pid));
}
